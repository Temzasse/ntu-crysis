...
def send_mailv4_to_responseunit(incident, recipient_list):
    ...
    map_url = "https://maps.googleapis.com/maps/api/staticmap?center=" + str(incident.latitude) + "," + str(
        incident.longitude) + "&zoom=14&size=400x400&markers=color:red%7Clabel:S%7C" + str(incident.latitude) + "," + str(incident.longitude)
    d = Context({
        'incident': incident,
        'time': time,
        'area': area,
